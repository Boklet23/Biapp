-- BiApp: Nullstill og bygg schema på nytt
-- Kjør dette i Supabase SQL Editor

-- ─────────────────────────────────────────────
-- RYDDE OPP (drop alt eksisterende)
-- ─────────────────────────────────────────────
drop table if exists swarm_reports cascade;
drop table if exists beekeeper_associations cascade;
drop table if exists hive_disease_flags cascade;
...
drop type if exists hive_type cascade;
drop type if exists subscription_tier cascade;
drop type if exists team_role cascade;
drop type if exists disease_severity cascade;
...
create policy "hives: les egne" on hives for select using (auth.uid() = user_id);
create policy "hives: les via team" on hives for select
  using (exists (
    select 1 from team_members tm
    join profiles p on p.team_id = tm.team_id
    where tm.user_id = auth.uid() and p.id = hives.user_id and tm.role in ('owner', 'admin')
  ));
